...
<p align="center">
  <img src="vs-reflector-manager.png" alt="vsReflector Manager" width="160"/>
</p>
...
chmod +x vs-reflector-manager
./vs-reflector-manager
```
...
```bash
cp vs-reflector-manager.desktop ~/.local/share/applications/
```
...
Both the generated mirrorlist and Chaotic AUR changes are written using `pkexec sh -c` — a polkit authentication dialog will appear before any write to `/etc/pacman.d/`.
